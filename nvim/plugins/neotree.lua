return {
  ...
  opts = function()
    ...
    local global_commands = {
      ...
      paste_file_from_clipboard = function(state)
        local node = state.tree:get_node()
        local dest_dir = node:get_id()
        if vim.fn.isdirectory(dest_dir) == 0 then dest_dir = vim.fn.fnamemodify(dest_dir, ":h") end
        vim.cmd.cd(dest_dir)
        out = io.popen "xclip-pastefile 2>&1"
        vim.cmd["cd!"] "-"
      end,

      copy_file_to_clipboard = function(state)
        local node = state.tree:get_node()
        local src_path = node:get_id()
        vim.notify("Copied: " .. src_path)
        out = io.popen("xclip-copyfile " .. src_path .. " 2>&1")
      end,
    }
    ...
    return {
      ...
      window = {
        ...
        mappings = {
          ...
          l = "child_or_open",
          -- Y = "copy_selector",
          y = "copy_file_to_clipboard",
          p = "paste_file_from_clipboard",
        },
      },
      filesystem = {
